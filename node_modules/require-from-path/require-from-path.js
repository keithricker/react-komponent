...
function requireFromPath(dir, request) {
  const makeIt = (xdir, checked) => {
    let xRequire = requireFromPath.cache && requireFromPath.cache.get(xdir);
    ...
    if (!xRequire) {
      ...
      try {
        ...
        throw new Error(`require-from-path: stat '${xdir}' failed: ${e.message}`);
      }

      if (!stat || !stat.isDirectory()) {
        if (checked) throw new Error(`require-from-path: not a directory: '${dir}'`);
        return makeIt(Path.dirname(xdir), true);
      }

      xRequire = createRequireFromPath(Path.join(xdir, "._require-from-path_"), xdir);

      requireFromPath.cache && requireFromPath.cache.set(xdir, xRequire);
    }
