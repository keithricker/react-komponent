...
const RootClass = KomponentClass.RootClass
let RCExtension; RootClass.extension((ext) => RCExtension = ext)
const DOM = KomponentClass.DOM
const { connect } = RootClass
...
class Render extends Object {
   constructor(obj) { super(obj); return proto.set(obj,proto.set(clone(Render.prototype),proto.get(obj))) }
   has(prop) { return this.hasOwnProperty(prop) }
}
class Renders extends Array { 
   get last() { return this[this.length-1] }; 
   get isComplete() { return this.last.completed }
   complete() { this.last.completed = true };
   push(...rends) { 
      return rends.forEach(rend => Array.prototype.push.call(this,new Render(rend)))
   }
   add(...arg) { return this.push(...arg) }
}

class SessionsMap extends WeakerMap { 
   set(...arg) { 
      let setted = WeakerMap.prototype.set.call(this,...arg).get(arg[0])
      setted.renders = new Renders(...setted.renders || [])
      proto.set(setted,proto.set({
         get hasRendered() { return this.renders.some(ren => ren.completed === true) }
      },proto.get(setted)))
   } 
}
let sessions = new SessionsMap()
let handlers = new WeakerMap()
let cPHandler = {}

const Komponent = function(comp) {
   let extension = extend(comp,function(props) {
      let thiss = props[rand] || props.component || this
      handlers.set(thiss['{{target}}'],thiss)
      sessions(thiss,{})
      return thiss
   }, isClass(comp) ? function(props) {

      let instance = proto.set(new React.Component(props),extension.prototype)
      if (extension.prototype.mapStateToProps) {

         Object.defineProperty(instance,'mapStateToProps',{ value:undefined,writable:true,configurable:true })

         let mapState = extension.prototype.mapStateToProps
         let mapDis = extension.prototype.mapDispatchToProps

         let mapped; let dis
         try { mapped = mapState.call(instance) } catch {}
         try { dis = mapDis.call(instance) } catch {}
         if (typeof mapped === 'function') mapState = mapped.bind(instance)
         if (typeof mapDis === 'function') mapDis = dis.bind(instance)

         let connector = Connector(mapState,mapDis)(extension)
         props = { ...connector.connectedProps(props),passThrough:true }
      }  

      let instanceProx = getCompProxy(instance)

      Object.keys(cPHandler.set).filter(key => { return (key in comp.prototype) && key !== 'mapStateToProps' }).forEach(key => {
         let instanceProp = Object.getOwnPropertyDescriptor(key,instance) || undefined
         let lvl = getLevel(comp.prototype,lvl => lvl.hasOwnProperty(key))
         ...
         if (!instanceProp) delete instanceProx[key]
         else Object.defineProperty(instanceProx,key,instanceProp)
      })
      return {...props,[rand]: instanceProx }
   } : undefined)
   ...
   function getCompProxy(thiss,props,bindr) {
      ...
      const safe = (ob,prop,binder) => {
         ...
         let returnVal
         if (theProp.originalValue) returnVal = prop.originalValue(binder)
         else {
      ...
      function setVal(ob,key,val) { Object.defineProperty(ob,key,{ value:val, configurable:true, writable:true, enumerable:true }); return true }

      let cloned = cloneProxy(thiss,cPHandler)
      let cP = bindr || cloned

      const renders = () => sessions(cP,{}).renders

      function register(ob) {
         if (!ob.constructor.statefulComponents)
      ...
      cPHandler.get = { 
         ...
         domNode(ob) { return DOM(ob) },
         createEvent(ob) {
            return function(name) { return cP.domNode.createEvent(name) }
         },
         dispatchEvent(ob) { 
            return function(event) { return cP.domNode.dispatchEvent(event) } 
         },
         originalSetState(thiss) {
            return function setState(...args) {
               return callIt(thiss,'setState',cP,...args) 
            }
         },
         ...
         setState(thiss) {
            return function setState(...args) {
               ...
               callIt(thiss,'setState',cP,arguments[0],(st) => {
                  cb(st)
         ...
         componentWillUnmount(ob) {
            return function componentWillUnmount() {
               ...
               if (ob.componentWillUnmount) 
                  callIt(ob,'componentWillUnmount',cP)           
         ...
         componentDidMount(ob) {
            ...
            return function componentDidMount() {
               ...
               if (ob.componentDidMount) callIt(ob,'componentDidMount',cP)
            }
         },
         ...
         render(ob,key) {
            ...
            let thiss = cP
            let res; let rendered; let props = thiss.props;
            ...
            const renderFetchingPage = (com) => {
               ...
               let Rendered = function() { 
                  let renderee = isClass(comp) ? com.render : com.render || com
                  return renderee.call(thiss,props) 
               }
               console.log('readying to render <FetchingPage>')
               return <FetchingPage overlay={thiss.props.overlay} component={Rendered} fetching={(thiss.state && thiss.state.fetching) ? true : false}></FetchingPage>
            }

            if (isClass(comp)) return function() { return renderFetchingPage(ob) }

            return function render() { 
               const renders = sessions(thiss,{}).renders = sessions(thiss).renders || []
         
               let newRender = {completed:false}; let newerRender = {completed:false};
               renders.add(newRender)
               
               let callee = thiss.connector || comp;
               if (thiss.connector && !props[rand]) {
                  console.log('first',{...thiss.state})
                  props = {...props,[rand]:thiss}
                  try { renders.push(newerRender); rendered = renderFetchingPage(thiss.connector) } catch(err) { console.log('error',err) } finally { console.log('second',{...thiss.state}); newerRender.completed = true; }
               }
               if (thiss.connector) props = {...props,[rand]:thiss}

               if (!sessions(thiss).hasRendered)
                  try { return renderFetchingPage(callee) } catch(err) { console.log('there was an error',err) } finally { newRender.completed = true; console.log('third',{...thiss.state}); }
               try {
                  rendered = renderFetchingPage(callee)
               } catch(err) { 
                  console.log('errorz',err)
               } finally {
                  console.log('fifth',{...thiss.state},'ft',{...thiss})
         ...
         getState(thiss) { 
            return function getState() { return cP.constructor.store.getState() }
         },
         useState(thiss) {
            thiss = cP
            return function useState(initial) {
               initial = typeof initial === 'function' ? initial({ ...thiss.state}) : initial
               cP.originalSetState(initial)
               function setTheState(state) {
                  return cP.originalSetState(typeof state === 'function' ? state({ ...thiss.state}) : state)
               }
               ...
               return [{...initial},setTheState]
            }
         },
         konstructor() { return 'Komponent' },
         ...
         useSelector(thiss) { 
            ...
            return function useSelector(...cb) {
               ...
               let selector; let selectorFunc
               useSelectors.set(thiss,useSelectors.get(thiss) || [])
   
               if (arguments.length === 1) {
                  selectorFunc = arguments[0]
                  selector = selectors.get(selectorFunc) || createSelector(...cb,(things) => things)
                  selectors.set(selectorFunc,selectors.get(selectorFunc) || selector)
                  if (!subscriptions.get(selectorFunc))
                     subscribeToStore.call(thiss,thiss.constructor.store,selectorFunc,selector,'useSelector')
                  return selector.call(thiss,theState,thiss.props)
               }
   
               selector = subscriptions.get(useSelector) || createSelector(...cb,(things) => things)
               useSelectors.set(thiss,useSelectors.get(thiss) || [])
               ...
               if (theState === theSelected || theSelected === theState[thiss.componentName])
                     return theSelected
                  subscribeToStore.call(thiss,thiss.constructor.store,useSelector,selector,'useSelector end')
               return theSelected
            }
         },
         ...
         useDispatch(thiss) {
            ...
            return function useDispatch(...args) {
               ...
               if (thiss.inLoop || inPrevious) {
                  ...
                  console.log('previous',thiss.getStackTrace())
                  // return
               }
         ...
         defaultMapState(thiss) {
            ...
            return function defaultMapState(state) {
               return { 
                  mappedState: state[thiss.componentName || lowerFirst(thiss.name || thiss.constructor.name)] 
               }
            }
         },
   
         connect() { return connectFunc },
   
         actions(thiss) {
            if (thiss.actions) return callIt(thiss,'actions',cP)
            return defaultActions.call(cP)
         },
         ...
         selector() { return createSelector },
         deregister(ob) { return function() { return deregister(ob) } },
         ...
         getStackTrace() { let gst = getStackTrace; return function getStackTrace() { return gst() } },
         ...
         original(thiss) {
            return new Standin(thiss,{
               get: function(ob,prop) {
                  ...
                  level = getLevel(level, lvl => lvl.hasOwnProperty(prop)) || level
                  let returnVal
                  if (prop === 'original') {
                     privs.caller = level
                     try { returnVal = Reflect.get(ob,prop,ob) } catch(err) { console.log('error',err) } finally { if (privs.caller === level) privs.caller = ob }
                  }
         ...
         constructor(ob) { return ob.constructor },
         ['{{handler}}']() { return cPHandler },
         handler(ob) { return ob.handler || cPHandler },
         variant() { return 'getCompProxy' },
         socket(ob) {
         ...
         default(ob,key) {
            ...
            if (key in Object.prototype) return
             ...
             ob = ob['{{target}}'] || ob; let original; let result; let obj
   
             if (!cPHandler.get.hasOwnProperty(key)) {

               obj = getLevel(ob,lvl => lvl.hasOwnProperty(key))
               if (!obj) return
               if (obj === Object.prototype || obj === React.Component.prototype)
                  return typeof ob[key] === 'function' ? ob[key].bind(cP) : ob[key]
               ...
               if (typeof original === 'function' && key !== 'constructor' && !original.defined) {
                  def[type] = cloneFunc(original,function() {
                     ...
                     if ((key === 'mapStateToProps' || key === 'mapDisPatchToProps') && typeof propCall === 'function') {
                        return callIt(obj,propCall,cP,...arg)
                     }
                     return propCall
                  },cP)
                  def[type].originalValue = (binder) => { ReflecTie(Object.defineProperty({},key,{...def,[type]:original}),key,binder) }
                  Object.defineProperty(def[type],'originalProp',{get:function() { return original }})
       ...
       cPHandler.set = { 
          ...
          state(ob,prop,val) { 
             if (!isClass(comp)) {
                ...
                if (initString && (initString === valString) && thiss.state)
                   return true
                ...
                if (!thiss.hasOwnProperty('state') || thiss.state === undefined || thiss.state === null)
                   ob.state = val

               if (!ob.suppressSetState) 
                  ob.state = val
             }
             ...
             Object.keys(fromLocal).forEach(key => {
                ...
                if (parent.reducers[key] || key === thiss.componentName) {
                   replenish[key] = fromLocal[key]; return
                }
             ...
             if (Object.keys(untracked).length > 1)
                parent.addReducers(untracked)
   
             if (isClass(comp)) ob.state = val
             return true
             },

             mapStateToProps(ob,key,val) {
               let thiss = cP
               if (ob.connector || proto.get(thiss).mapStateToProps || props.passThrough || (ob.hasOwnProperty('mapStateToProps') && ob.mapStateToProps === undefined))
                  return true
               const parent = isClass(comp) ? thiss.constructor : comp
               let mapState; let mapDis
               if (typeof val.call(thiss) === 'function')
                  mapState = val.call(thiss)
               mapState = mapState.bind(thiss)
               if (ob.mapDispatchToProps) mapDis = ob.mapDispatchToProps.bind(thiss)
               
               Object.defineProperty(ob,'mapStateToProps',{ value:undefined,writable:true,configurable:true })
               ob.connector = Connector(mapState,mapDis)(parent)
               return true
             },
             ...
             reducer(ob) {
                ...
                if ((!Parent.customReducers) || allowed) {
                   ...
                   Parent.store.replaceReducer(combineReducers({ ...RCExtension.reducers, ...Parent.reducers }))
                }
             },
             ...
             sagas(ob) { 
               ...
               register(ob); ob.constructor.refreshSagas() 
             },

             default(ob,key,val) {
               let thisProto = getLevel(ob,lvl => lvl.hasOwnProperty('constructor') && lvl.constructor === ob.constructor && lvl !== ob)
               if (typeof val === 'function' && !thisProto.hasOwnProperty(key))
                  setVal(thisProto,key,val.bind(cP))
      ...
      function initialize(ob) {
         ...
         if (!Parent.reducers.hasOwnProperty(componentName)) {
            ...
            if (rootReducer && !rootReducer.hasOwnProperty(componentName) && !ob.initialized)
               onSets.reducer(ob)
         }
         if (('sagas' in ob) && !Parent.sagas[componentName])
            Parent.refreshSagas() 
      }
   }
   
}
proto.set(Komponent,KomponentClass)
