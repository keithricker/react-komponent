...
export default function Komponent(klass) {
  const Klass = klass
  let Mything = <Klass anotherProp="prop"></Klass>
  klass.render = function() { return Mything }
  return klass
}
...
export class Component extends React.Component {
    constructor(props) {
        ...
        const originalRender = thiss.render
        this.props = { ...this.props,stuff:'things'}
        this.render = function() {
           const ThisComponent = () => {
              return <div>{originalRender.call(thiss)}</div>
           }
           return <ThisComponent></ThisComponent>
        }
    }
    useSelector(cb) { 
    ...
    get reducer() {
       ...
       const reducerFunction = (state=thiss.state,action) => { 
        ...
        return (state=thiss.state,action) => {
           ...
           console.log("EQUAL", state === newState)
           console.log('state',state, 'new',newState)

           suppressSetState = ((newState === state) || (action.type.includes('@@'))) ? true : false
